"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RoundsModule = void 0;
var common_1 = require("@nestjs/common");
var typeorm_1 = require("@nestjs/typeorm");
var rounds_controller_1 = require("../controllers/rounds.controller");
var rounds_service_1 = require("../services/rounds.service");
var rounds_entity_1 = require("../entityes/rounds.entity");
var token_middleware_1 = require("../middlewares/token.middleware");
var RoundsModule = /** @class */ (function () {
    function RoundsModule() {
    }
    RoundsModule.prototype.configure = function (consumer) {
        consumer
            .apply(token_middleware_1.TokenMiddleware)
            .forRoutes('rounds');
    };
    RoundsModule = __decorate([
        (0, common_1.Module)({
            imports: [typeorm_1.TypeOrmModule.forFeature([rounds_entity_1.Rounds])],
            controllers: [rounds_controller_1.RoundsController],
            providers: [rounds_service_1.RoundsService],
            exports: [rounds_service_1.RoundsService],
        })
    ], RoundsModule);
    return RoundsModule;
}());
exports.RoundsModule = RoundsModule;
